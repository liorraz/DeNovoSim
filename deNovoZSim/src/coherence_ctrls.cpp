...
void DeNovoImpl::init(MemObject* _parent, Network* network, const char* name){
	...
	parentRTT = (network) ? network->getRTT(name, _parent->getName()) : 0;
}



uint64_t DeNovoImpl::processAccess(Address lineAddr, uint32_t lineId, uint32_t numLines, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, MemObject* parentLLCObj) {
...
//	info("###### %s at init with parent index: %u and parent name %s" ,name, p, _parents[p]->getName()); 
...
//    uint64_t respCycle = cycle;
//    MESIState* state = &array[lineId];
//    //info("address: %lx , line id: %d , access type: %s , state: %s", lineAddr, lineId, AccessTypeName(type), MESIStateName(*state)); 
//    switch (type) {
//        // A PUTS/PUTX does nothing w.r.t. higher coherence levels --- it dies here
//        case PUTS: //Clean writeback, nothing to do (except profiling)
//            assert(*state != I);
